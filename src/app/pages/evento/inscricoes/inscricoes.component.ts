...
export class InscricoesComponent implements OnInit, AfterViewInit {
    ...
    async ngOnInit() {
        this.evento = await this.eventoService.detalha(
    ...
    async cancelar(inscricao: InscricaoEvento) {
        this.dialogService.confirmacao(
            ...
            this.acaoService.executa(async () => {
                await this.eventoService.cancelaInscricao(this.evento.id, inscricao.id).toPromise();
    ...
    async confirmar(inscricao: InscricaoEvento) {
        await this.eventoService.confirmaInscricao(this.evento.id, inscricao.id).toPromise();
