...
export class ConsultaComponent implements AfterViewInit {
    classificados: BuscaPaginada<Noticia>;
    ...
    async busca(event = {pagina: 1, tamanhoPagina: 10}) {
        this.classificados = await this.loader.listen(this.noticiaService.consulta(
            'CLASSIFICADOS',
            event.pagina,
            event.tamanhoPagina
        )).toPromise();
    ...
    async excluir(noticia: Noticia) {
        this.dialogService.confirmacao(
            ...
            'global.nao',
            {
                elemento: this.translateService.instant('global.elemento.classificados'),
