...
export class EdicaoComponent extends AbstractFormComponent<Noticia> implements AfterViewInit {
  ...
  constructor(
    ...
    this.entidade.tipo = 'CLASSIFICADOS';
  }
  ...
  async salvar() {
    if (this.classificados.id) {
      await this.noticiaService.atualiza(this.classificados).toPromise();
    } else {
      await this.noticiaService.cadastra(this.classificados).toPromise();
    }
