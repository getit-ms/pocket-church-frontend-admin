...
export class ConsultaComponent implements AfterViewInit {
  ...
  async busca() {
    this.calendarios = await this.loader.listen(this.atendimentoService.consulta()).toPromise();
  }
  ...
  async excluir(calendario: CalendarioAtendimento) {
    this.dialogService.confirmacao(
      ...
      'global.nao',
      {
        ...
        descricao: calendario.gerente.nome
      }
    ).subscribe(() => {
      this.acaoService.executa(async () => {
        await this.atendimentoService.remove(calendario.id).toPromise();
