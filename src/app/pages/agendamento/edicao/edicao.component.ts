...
export class EdicaoComponent extends AbstractFormComponent<CalendarioAtendimento> implements AfterViewInit {
    ...
    constructor(
        ...
        this.atendimentoService.buscaGerentes()
            .subscribe(gerentes => {
    ...
    protected async carregaEntidade(id: any) {
        this.entidade = await this.atendimentoService.detalha(id).toPromise();
        this.gerentes = [this.entidade.gerente];
        this.hoje();
    }

    async adicionarHorario(horario: HorarioAtendimento) {
        await this.atendimentoService.cadastraHorario(
            this.calendario.id,
            horario
        ).toPromise();
    ...
    async salvar() {
        ...
        this.entidade = await this.atendimentoService.cadastra(this.calendario).toPromise();
    ...
    private async buscaEventos() {
        ...
        let observable = this.atendimentoService.buscaHorarios(
            this.calendario.id,
        ...
        let agendamentos = await this.loader.listen(this.atendimentoService.buscaAgendamentos(
            this.calendario.id,
            new Date(this.viewDate.getFullYear(), this.viewDate.getMonth(), this.viewDate.getDate() - this.viewDate.getDay()),
            new Date(this.viewDate.getFullYear(), this.viewDate.getMonth(), this.viewDate.getDate() + (7 - this.viewDate.getDay()))
        )).toPromise();
