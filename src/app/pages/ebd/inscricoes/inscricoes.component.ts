...
export class InscricoesComponent implements OnInit, AfterViewInit {
    ...
    async ngOnInit() {
        this.ebd = await this.eventoService.detalha(
    ...
    async cancelar(inscricao: InscricaoEvento) {
        this.dialogService.confirmacao(
            ...
            this.acaoService.executa(async () => {
                await this.eventoService.cancelaInscricao(this.ebd.id, inscricao.id).toPromise();
    ...
    async confirmar(inscricao: InscricaoEvento) {
        await this.eventoService.confirmaInscricao(this.ebd.id, inscricao.id).toPromise();
